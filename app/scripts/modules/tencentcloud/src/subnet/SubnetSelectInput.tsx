...
export class SubnetSelectInput extends React.Component<ISubnetSelectInputProps, ISubnetSelectInputState> {
  ...
  private isClassicLockout(region: string, credentials: string, application: Application): boolean {
    const { classicLaunchLockout, classicLaunchAllowlist: whitelist } = TENCENTCLOUDProviderSettings;
    ...
    const isWhitelisted =
      !!whitelist &&
      whitelist.some(
        (w: { region: string; credentials: string }) => w.region === region && w.credentials === credentials,
      );
