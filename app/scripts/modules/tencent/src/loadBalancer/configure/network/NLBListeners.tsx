...
import { NLBListenerProtocol, IListenerDescription, ITencentNetworkLoadBalancerUpsertCommand } from 'tencent/domain';
...
export class NLBListeners extends React.Component<INLBListenersProps>
  ...
  public validate(values: ITencentNetworkLoadBalancerUpsertCommand) {
    const errors = {} as any;

    // Check to make sure all target groups have an associated listener
    const targetGroupNames = values.targetGroups.map((tg: { name: string }) => tg.name);
      ...
      <div className="container-fluid form-horizontal">
        <div className="form-group">
          <div className="col-md-12">
            {values.listeners.map((listener: IListenerDescription, index: number) => (
                        ...
                        <tbody>
                          <tr className="not-sortable">
                            ...
                            <td>
                              ...
                              >
                                <option value="" />
                                {uniq(values.targetGroups.map((tg: { name: string }) => tg.name)).map(
                                  (name: string) => (
