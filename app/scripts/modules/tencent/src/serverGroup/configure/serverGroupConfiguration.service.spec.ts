...
describe('Service: tencentServerGroupConfiguration', function() {
  let service: TencentServerGroupConfigurationService,
    $q: IQService,
  ...
  beforeEach(mock.module(TENCENT_SERVER_GROUP_CONFIGURATION_SERVICE));

  beforeEach(
    mock.inject(function(
      _tencentServerGroupConfigurationService_: TencentServerGroupConfigurationService,
      _$q_: IQService,
      ...
      service = _tencentServerGroupConfigurationService_;
      $q = _$q_;
  ...
  describe('configureSecurityGroupOptions', function() {
    beforeEach(function() {
      ...
      this.command = {
        backingData: {
          ...
          filtered: {
            securityGroups: this.allSecurityGroups.test.tencent['us-west-1'],
          },
        },
    ...
    it('updates filteredData to new region - no VPC', function() {
      const expected = this.allSecurityGroups.test.tencent['us-east-1'].slice(0, 2);
      this.command.region = 'us-east-1';
    ...
    it('updates filteredData to new VPC', function() {
      const expected = this.allSecurityGroups.test.tencent['us-west-1'].slice(3, 5);
      this.command.vpcId = 'vpc-1';
      service.configureSecurityGroupOptions(this.command);
      expect(this.command.backingData.filtered.securityGroups).toEqual(expected);
    });
