...
import { IKeyPair, ITencentLoadBalancerSourceData, IScalingProcess, ICLBListener } from 'tencent/domain';
...
export class TencentServerGroupConfigurationService {
    ...
    return Promise.all(
      command.backingData.filtered.lbList.map(flb =>
        this.loadBalancerReader
          ...
          .then((loadBalancers: ITencentLoadBalancerSourceData[]) => ({
...
export const TENCENT_SERVER_GROUP_CONFIGURATION_SERVICE = 'spinnaker.tencent.serverGroup.configure.service';
module(TENCENT_SERVER_GROUP_CONFIGURATION_SERVICE, [
  SECURITY_GROUP_READER,
  require('tencent/instance/tencentInstanceType.service').name,
  LOAD_BALANCER_READ_SERVICE,
  CACHE_INITIALIZER_SERVICE,
  SERVER_GROUP_COMMAND_REGISTRY_PROVIDER,
]).service('tencentServerGroupConfigurationService', TencentServerGroupConfigurationService);
