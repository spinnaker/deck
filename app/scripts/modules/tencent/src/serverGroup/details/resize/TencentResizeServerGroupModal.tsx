...
import { TencentModalFooter } from 'tencent/common';
import { ITencentAsg, ITencentServerGroup } from 'tencent/domain';

export interface ITencentResizeServerGroupModalProps extends IModalComponentProps {
  application: Application;
  serverGroup: ITencentServerGroup;
}

export interface ITencentResizeServerGroupModalState {
  advancedMode: boolean;
  interestingHealthProviderNames: string[]; // managed by a separate component
  initialValues: ITencentResizeServerGroupValues;
  taskMonitor: TaskMonitor;
  platformHealthOnlyShowOverride?: boolean;
}

export interface ITencentResizeServerGroupValues {
  min: number;
  max: number;
  desired: number;
...
export class TencentResizeServerGroupModal extends React.Component<
  ITencentResizeServerGroupModalProps,
  ITencentResizeServerGroupModalState
> {
  public static defaultProps: Partial<ITencentResizeServerGroupModalProps> = {
    closeModal: noop,
    dismissModal: noop,
  };

  private formikRef = React.createRef<Formik<ITencentResizeServerGroupValues>>();

  public static show(props: ITencentResizeServerGroupModalProps): Promise<IResizeJob> {
    const modalProps = {};
    return ReactModal.show(TencentResizeServerGroupModal, props, modalProps);
  }

  constructor(props: ITencentResizeServerGroupModalProps) {
    super(props);
  ...
  private getChangedFields(capacity: ICapacity, asg: ITencentAsg): IChangedField[] {
  ...
  private validate = (
    values: ITencentResizeServerGroupValues,
  ): Partial<FormikErrors<ITencentResizeServerGroupValues>> => {
    const { min, max, desired } = values;
    const { asg } = this.props.serverGroup;
    const errors: Partial<FormikErrors<ITencentResizeServerGroupValues>> = {};
  ...
  private submit = (values: ITencentResizeServerGroupValues): void => {
  ...
  private renderSimpleMode(formik: FormikProps<ITencentResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { asg } = serverGroup;
    return (
      <div>
  ...
  private renderAdvancedMode(formik: FormikProps<ITencentResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
    const { errors } = formik;
    const { asg } = serverGroup;
          ...
          <FormikFormField
            ...
            input={props => (
              <>
                ...
                <HelpField id="tencent.serverGroup.capacityConstraint" />
              </>
            )}
          />
        </div>
  ...
  private renderScalingPolicyWarning(formik: FormikProps<ITencentResizeServerGroupValues>): JSX.Element {
    const { serverGroup } = this.props;
  ...
  public render() {
    ...
    return (
      <>
        ...
        <Formik<ITencentResizeServerGroupValues>
          ref={this.formikRef}
