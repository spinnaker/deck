...
import { ITencentServerGroup, ITencentServerGroupView } from 'tencent/domain';
import { TencentCloneServerGroupModal } from 'tencent/serverGroup/configure/wizard/TencentCloneServerGroupModal';
import { TencentReactInjector } from 'tencent/reactShims';
import { ITencentServerGroupCommand } from '../configure';
import { TencentResizeServerGroupModal } from './resize/TencentResizeServerGroupModal';

export interface ITencentServerGroupActionsProps extends IServerGroupActionsProps {
  serverGroup: ITencentServerGroupView;
}

export class TencentServerGroupActions extends React.Component<ITencentServerGroupActionsProps> {
  ...
  private rollbackServerGroup = (): void => {
    ...
    let serverGroup: ITencentServerGroup = this.props.serverGroup;
    let previousServerGroup: ITencentServerGroup;
    ...
    if (serverGroup.isDisabled) {
       ...
       */
      serverGroup = orderBy(
        allServerGroups.filter((g: ITencentServerGroup) => g.name !== previousServerGroup.name && !g.isDisabled),
        ['instanceCounts.total', 'createdTime'],
        ['desc', 'desc'],
      )[0] as ITencentServerGroup;
    }

    // the set of all server groups should not include the server group selected for rollback
    allServerGroups = allServerGroups.filter((g: ITencentServerGroup) => g.name !== serverGroup.name);
  ...
  private resizeServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    TencentResizeServerGroupModal.show({ application: app, serverGroup });
  };

  private cloneServerGroup = (): void => {
    const { app, serverGroup } = this.props;
    TencentReactInjector.tencentServerGroupCommandBuilder
      .buildServerGroupCommandFromExisting(app, serverGroup)
      .then((command: ITencentServerGroupCommand) => {
        const title = `Clone ${serverGroup.name}`;
        TencentCloneServerGroupModal.show({ title, application: app, command });
      });
  };
