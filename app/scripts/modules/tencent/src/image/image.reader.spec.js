...
import { TencentImageReader } from './image.reader';

describe('Service: tencent Image Reader', function() {
  var service, $http, scope;

  beforeEach(
    window.inject(function($httpBackend, $rootScope) {
      service = new TencentImageReader();
      $http = $httpBackend;
      scope = $rootScope.$new();
    }),
  );
  ...
  describe('findImages', function() {
    ...
    function buildQueryString() {
      return API.baseUrl + '/images/find?provider=tencent&q=' + query + '&region=' + region;
    }
  ...
  describe('getImage', function() {
    ...
    function buildQueryString() {
      return [API.baseUrl, 'images', credentials, region, imageName].join('/') + '?provider=tencent';
    }
