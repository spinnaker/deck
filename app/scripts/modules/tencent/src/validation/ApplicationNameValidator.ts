...
class TencentApplicationNameValidator implements IApplicationNameValidator {
  ...
  private validateClassicLock(warnings: string[]): void {
    const lockoutDate = TencentProviderSettings.classicLaunchLockout;
    if (lockoutDate && lockoutDate < new Date().getTime()) {
      warnings.push(
        `New applications deployed to Tencent are restricted to VPC; you cannot create server groups,
...
ApplicationNameValidator.registerValidator('tencent', new TencentApplicationNameValidator());
