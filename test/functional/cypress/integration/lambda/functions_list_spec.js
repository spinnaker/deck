...
describe('Amazon Lambda: Lambda functions test', () => {
  ...
  beforeEach(() => {
    ...
    cy.route('/functions?account=aws-cy-test&functionName=lambdait-java-integration-test&provider=aws&region=us-west-2', 'fixture:lambda/functions/functions_list.json')
  ...
  it('shows the Lambda funtions list', () => {
    ...
    //Check the details
    cy.get('h4.collapsible-heading').should(($h) => {
    ...
    //Create Lambda
    cy.get('.application-actions').find('button').should(($b) => {
    ...
    cy.get('tr.MapPair')
    .children('td')
    .find('input.form-control.input.input-sm').each((item, index, list) => {
